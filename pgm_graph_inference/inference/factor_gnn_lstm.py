...
class GGNN_LSTM(nn.Module):
    def __init__(self, state_dim, message_dim,hidden_unit_message_dim, hidden_unit_readout_dim, n_steps=10):
        ...
        self.propagator1 = nn.LSTMCell(self.message_dim, self.state_dim)
        self.propagator2 = nn.LSTMCell(self.message_dim, self.state_dim)
    ...
    def forward(self, J, b):
        ...
        factor_cell_state = torch.zeros(n_factors, self.state_dim).to(J.device)
        var_cell_state = torch.zeros(n_var_nodes, self.state_dim).to(J.device)
        ...
        for step in range(self.n_steps):
            ...
            factor_hidden_states, factor_cell_state = self.propagator1(node_messages, (factor_hidden_states, factor_cell_state))
            ...
            node_messages = scatter(edge_messages, col, dim=0, reduce='sum')
            var_hidden_states, var_cell_state = self.propagator2(node_messages, (var_hidden_states, var_cell_state))
